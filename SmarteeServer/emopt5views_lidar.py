# Forked from emopt5views.py — the "Take 5 Pictures" capture flow's own
# reconstruction engine, kept separate so LiDAR depth trial-and-error here
# can never affect emopt5views.py (used by the "Upload 5 Photos" flow).
# Currently identical to emopt5views.py; depth-residual work lands here only.
...
from seg.lidar_tooth_pose_constraints import LiDARToothPoseConfiguration
from seg.lidar_constraint_diagnostics import write_lidar_h5_diagnostics

print = functools.partial(print, flush=True)
LIDAR_TOOTH_SHAPE_WEIGHT = 0.002
METRES_SQUARED_TO_MILLIMETRES_SQUARED = 1_000_000.0


class EMOpt5Views(object):
    @staticmethod
    def _lower_arch_split_index(visible_indices, num_upper_tooth):
        # Index into this view's visible-tooth list where lower-arch teeth
        # start. Frontal/lateral photos show both arches, so this is a real
        # split point; an occlusal photo shows only ONE arch, so every
        # index can be upper-only (nothing >= num_upper_tooth) or
        # lower-only (everything >= num_upper_tooth) - handle both instead
        # of crashing on an empty reduction.
        lower_positions = np.argwhere(visible_indices >= num_upper_tooth)
        if lower_positions.size == 0:
            return len(visible_indices)
        return lower_positions.min()
    ...
    def __init__(
        ...
        empty_photos = [PHOTO(ph).name for ph, v in enumerate(self.P_true) if len(v) == 0]
        if empty_photos:
            # ponytail: fails fast here (the one place all 5 photos funnel through)
            # instead of letting Open3D crash later with a cryptic native error.
            raise ValueError(
                f"No teeth boundary detected in photo(s): {', '.join(empty_photos)}. "
                "Retake with better framing/lighting, or provide a hand-edited edge mask."
            )
        self.P_true_normals = [
        ...
        self.ul_sp = {
            phType.value: self._lower_arch_split_index(
                self.visIdx[phType.value], self.numUpperTooth
            )
            for phType in [PHOTO.UPPER, PHOTO.LOWER, PHOTO.LEFT, PHOTO.RIGHT, PHOTO.FRONTAL]
        }
        ...
        self.weightFeatureVec = float(
            shape_regularization
        )  # weight of the standardized PCA shape-coefficient penalty
        ...
        # LiDAR is opt-in and remains entirely isolated to this engine.  These
        # values are populated by main.py only after the strict capture gate.
        self.lidar_constraints = {}
        self.lidar_correspondences = {}
        self.lidar_metric_scale = LiDARConstraintConfiguration().metres_per_ssm_unit_initial
        self.lidar_diagnostics = {"enabled": False, "reason": "no_eligible_constraint"}
        self.lidar_tooth_pose_constraints = {}
        self.lidar_tooth_pose_correspondences = {}
        self.lidar_tooth_pose_initial_trans_vecs = None
        self.lidar_tooth_pose_initial_rot_vecs = None
        self.lidar_tooth_pose_diagnostics = {"enabled": False, "reason": "no_eligible_constraint"}
        self.lidar_tooth_shape_constraints = {}
        self.lidar_tooth_shape_correspondences = {}
        self.lidar_tooth_shape_initial_feature_vec = None
        self.lidar_tooth_shape_activation_allowed = False
        self.lidar_tooth_shape_diagnostics = {"enabled": False, "reason": "m6_not_ready"}

    def set_lidar_constraints(self, constraints, skipped=None):
        """Attach independently gated direct-view clouds to global stages only."""

        self.lidar_constraints = dict(constraints or {})
        self.lidar_correspondences = {}
        skipped = dict(skipped or {})
        if self.lidar_constraints:
            configuration = next(iter(self.lidar_constraints.values())).configuration
            self.lidar_metric_scale = configuration.metres_per_ssm_unit_initial
            self.lidar_diagnostics = {
                "enabled": True,
                "weight": configuration.weight,
                "metricScaleMetresPerSSMUnit": self.lidar_metric_scale,
                "eligibleViews": sorted(constraint.field for constraint in self.lidar_constraints.values()),
            ...
            self.lidar_diagnostics = {
                "enabled": False,
                "reason": "no_eligible_constraint",
                "skippedViews": skipped,
            }

    def set_lidar_tooth_pose_constraints(self, constraints, skipped=None):
        """Attach already-gated field-local tooth constraints for Stage 2 only."""

        self.lidar_tooth_pose_constraints = dict(constraints or {})
        self.lidar_tooth_pose_correspondences = {}
        self.lidar_tooth_pose_initial_trans_vecs = None
        self.lidar_tooth_pose_initial_rot_vecs = None
        self.lidar_tooth_shape_constraints = {}
        ...
        self.lidar_tooth_shape_diagnostics = {"enabled": False, "reason": "m6_not_ready"}
        skipped = dict(skipped or {})
        if self.lidar_tooth_pose_constraints:
            configuration = next(iter(next(iter(self.lidar_tooth_pose_constraints.values())))).configuration
            eligible_slots = {}
            for values in self.lidar_tooth_pose_constraints.values():
                for constraint in values:
                    entry = eligible_slots.setdefault(
                        constraint.slot_id,
                        {"fields": [], "pointCount": 0, "contributingKeyframes": []},
                    )
                    entry["fields"].append(constraint.field)
                    entry["pointCount"] += int(len(constraint.points_k0_metres))
                    entry["contributingKeyframes"].append(list(constraint.contributing_keyframes))
            for entry in eligible_slots.values():
                entry["fields"].sort()
            self.lidar_tooth_pose_diagnostics = {
                "enabled": True,
                "weight": configuration.weight,
    ...
    def begin_lidar_tooth_pose_diagnostics(self):
        """Snapshot the selected Stage-1 pose before M6 can adjust individual teeth."""

        if self.lidar_tooth_pose_initial_trans_vecs is None:
            self.lidar_tooth_pose_initial_trans_vecs = np.asarray(
                self.transVecXYZs, dtype=np.float64
            ).copy()
            self.lidar_tooth_pose_initial_rot_vecs = np.asarray(
                self.rotVecXYZs, dtype=np.float64
            ).copy()

    def finalize_lidar_tooth_pose_diagnostics(self):
        """Record only parameter norms, never patient geometry, for the final M6 pose."""

        if not self.lidar_tooth_pose_constraints:
            return
        self.begin_lidar_tooth_pose_diagnostics()
        original_indices = np.flatnonzero(self.Mask)
        deltas = {}
        for original_index, constraints in self.lidar_tooth_pose_constraints.items():
            matches = np.flatnonzero(original_indices == original_index)
            if len(matches) != 1 or not constraints:
                continue
            tooth_index = int(matches[0])
            translation_delta = np.asarray(
                self.transVecXYZs[tooth_index], dtype=np.float64
            ) - self.lidar_tooth_pose_initial_trans_vecs[tooth_index]
            rotation_delta = np.asarray(
                self.rotVecXYZs[tooth_index], dtype=np.float64
            ) - self.lidar_tooth_pose_initial_rot_vecs[tooth_index]
            translation_ssm_units = float(np.linalg.norm(translation_delta))
            deltas[constraints[0].slot_id] = {
                "translationSSMUnits": translation_ssm_units,
                "translationMillimetres": float(
                    translation_ssm_units * self.lidar_metric_scale * 1000.0
                ),
                "rotationRadians": float(np.linalg.norm(rotation_delta)),
            }
        self.lidar_tooth_pose_diagnostics["poseDeltaNorms"] = deltas
        self.lidar_tooth_pose_diagnostics["scaleParameterDeltaFromM6Loss"] = 0.0
        self.lidar_tooth_pose_diagnostics["stage3FeatureVectorDeltaFromM6Loss"] = 0.0

    def set_lidar_photo_contour_diagnostics(self, before, after):
        """Attach the photo-only objective before and after the coarse M5 stages."""

        before = float(before)
        after = float(after)
        self.lidar_diagnostics["photoContourLossBefore"] = before
        self.lidar_diagnostics["photoContourLossAfter"] = after
        if before != 0.0:
            self.lidar_diagnostics["photoContourLossChangePercent"] = float(
                100.0 * (after - before) / before
            )

    def _lidar_tooth_camera_points(
        self, constraint, scales, rot_vecs, trans_vecs, deformed_points=None
    ):
        original_indices = np.flatnonzero(self.Mask)
        matches = np.flatnonzero(original_indices == constraint.original_tooth_index)
        if len(matches) != 1:
            return None, None
        tooth_index = int(matches[0])
        centroid = self.X_Mu_centroids[tooth_index]
        rotation = self.computeRotMats(rot_vecs[[tooth_index]])[0]
        tooth_points = self.X_deformed[tooth_index] if deformed_points is None else deformed_points
        local_points = (
            scales[tooth_index] * np.matmul(tooth_points - centroid, rotation)
            + trans_vecs[tooth_index]
            + centroid
        )
        world_points = local_points
        lower_in_shared_view = tooth_index >= self.numUpperTooth and constraint.photo_index in (
            PHOTO.LEFT.value, PHOTO.RIGHT.value, PHOTO.FRONTAL.value
        )
        if lower_in_shared_view:
            world_points = self.updateLowerPointPosByRelaPose(
                local_points, self.updateRelaRotMat(self.rela_rxyz), self.rela_txyz
            )
        camera = self.updatePointPosInCameraCoord(
            world_points,
            self.updateExtrinsicViewMatrix(
                self.ex_rxyz[constraint.photo_index], self.ex_txyz[constraint.photo_index]
            ),
        )
        return camera, (tooth_index, local_points, lower_in_shared_view)

    def prepare_lidar_tooth_shape_correspondences(self):
        """Freeze only M6 pairs backed by two direct fields for Stage 3."""

        if not self.lidar_tooth_shape_activation_allowed:
            self.lidar_tooth_shape_constraints = {}
            self.lidar_tooth_shape_correspondences = {}
            reason = (
                "m5_candidate_rejected"
                if self.lidar_diagnostics.get("fallbackApplied")
                else "stage_two_not_complete"
            )
            self.lidar_tooth_shape_diagnostics = {
                "enabled": False,
                "reason": reason,
            }
            return
        ...
        for original_index, constraints in self.lidar_tooth_pose_constraints.items():
            ...
            for constraint in constraints:
                if not getattr(constraint, "calibrated_rgb_depth", False):
                    continue
                pair = self.lidar_tooth_pose_correspondences.get(
                    (original_index, constraint.field)
                )
                if pair is None or len(pair.model_indices) == 0:
                    continue
                usable.append(constraint)
            distinct_fields = sorted({constraint.field for constraint in usable})
            if len(distinct_fields) < 2:
                continue
            eligible[original_index] = tuple(usable)
            slot_id = usable[0].slot_id
            pair_counts = {}
            for constraint in usable:
                pair = self.lidar_tooth_pose_correspondences[
                    (original_index, constraint.field)
                ]
                frozen = FixedLiDARCorrespondences(
                    np.asarray(pair.model_indices, dtype=np.intp).copy(),
                    np.asarray(pair.lidar_indices, dtype=np.intp).copy(),
                )
                frozen_pairs[(original_index, constraint.field)] = frozen
                pair_counts[constraint.field] = int(len(frozen.model_indices))
            slot_diagnostics[slot_id] = {
                "fields": distinct_fields,
                "pairCounts": pair_counts,
            }

        self.lidar_tooth_shape_constraints = eligible
        self.lidar_tooth_shape_correspondences = frozen_pairs
        if not eligible:
            self.lidar_tooth_shape_diagnostics = {
                "enabled": False,
                "reason": "fewer_than_two_fields_with_m6_pairs",
                "eligibleSlots": {},
            }
            return
        if self.lidar_tooth_shape_initial_feature_vec is None:
            self.lidar_tooth_shape_initial_feature_vec = np.asarray(
                self.featureVec, dtype=np.float64
            ).copy()
        self.lidar_tooth_shape_diagnostics = {
            "enabled": True,
            "weight": LIDAR_TOOTH_SHAPE_WEIGHT,
            "eligibleSlots": slot_diagnostics,
            "featureVectorDeltaNorms": {},
        }

    def update_lidar_tooth_shape_diagnostics(self):
        """Record coefficient movement without exposing patient geometry."""

        if not self.lidar_tooth_shape_diagnostics.get("enabled"):
            return
        if self.lidar_tooth_shape_initial_feature_vec is None:
            return
        original_indices = np.flatnonzero(self.Mask)
        deltas = {}
        for original_index, constraints in self.lidar_tooth_shape_constraints.items():
            matches = np.flatnonzero(original_indices == original_index)
            if len(matches) != 1:
                continue
            tooth_index = int(matches[0])
            slot_id = constraints[0].slot_id
            delta = self.featureVec[tooth_index] - self.lidar_tooth_shape_initial_feature_vec[tooth_index]
            deltas[slot_id] = float(np.linalg.norm(delta))
        self.lidar_tooth_shape_diagnostics["featureVectorDeltaNorms"] = deltas

    def compute_lidar_tooth_shape_loss_and_gradient(
        self, feature_vec, pIdx, param_count, stage
    ):
        """Stage-3 Huber loss projected into only the matching PCA slice."""

        gradient = np.zeros((param_count,), dtype=np.float64)
        if stage != 3 or not self.lidar_tooth_shape_constraints:
            return 0.0, gradient
        losses = []
        ...
        for original_index, constraints in self.lidar_tooth_shape_constraints.items():
            ...
            if len(matches) != 1:
                continue
            tooth_index = int(matches[0])
            deformed = self.updateDeformedPointPos(
                feature_vec[[tooth_index]], np.array([tooth_index])
            )[0]
            tooth_gradient = np.zeros((self.numPC,), dtype=np.float64)
            field_count = 0
            for constraint in constraints:
                pair = self.lidar_tooth_shape_correspondences.get(
                    (original_index, constraint.field)
                )
                if pair is None or len(pair.model_indices) == 0:
                    continue
                camera, state = self._lidar_tooth_camera_points(
                    constraint,
                    self.scales,
                    self.rotVecXYZs,
                    self.transVecXYZs,
                ...
                if camera is None:
                    continue
                _, _, lower_in_shared_view = state
                model_indices = pair.model_indices
                aligned = arkit_points_from_em_camera(
                    camera[model_indices], self.lidar_metric_scale
                )
                loss, point_gradient = huber_point_to_surface_loss(
                    aligned,
                ...
                losses.append(loss)
                field_count += 1
                extrinsic = self.updateExtrinsicViewMatrix(
                    self.ex_rxyz[constraint.photo_index],
                    self.ex_txyz[constraint.photo_index],
                )
                local_gradient = (
                    point_gradient * axes * self.lidar_metric_scale
                ) @ extrinsic[:3, :].T
                if lower_in_shared_view:
                    local_gradient = local_gradient @ self.updateRelaRotMat(
                        self.rela_rxyz
                    ).T
                rotation = self.computeRotMats(self.rotVecXYZs[[tooth_index]])[0]
                deformed_gradient = (
                    self.scales[tooth_index] * local_gradient @ rotation.T
                )
                basis = self.SigmaT[tooth_index].reshape(
                    self.numPC, self.numPoint, 3
                )[:, model_indices, :]
                sqrt_eigenvalues = np.asarray(
                    self.SqrtEigVals[tooth_index], dtype=np.float64
                ).reshape(-1)
                basis = basis * sqrt_eigenvalues[:, None, None]
                tooth_gradient += np.sum(
                    basis * deformed_gradient[None, :, :], axis=(1, 2)
                )
            if field_count:
                start = pIdx["featureVec"] + tooth_index * self.numPC
                gradient[start : start + self.numPC] += tooth_gradient
        if not losses:
            return 0.0, gradient
        weight = LIDAR_TOOTH_SHAPE_WEIGHT
        factor = (
            weight
            * METRES_SQUARED_TO_MILLIMETRES_SQUARED
            / len(losses)
        )
        self.lidar_tooth_shape_diagnostics["lastLossMetresSquared"] = float(
            np.mean(losses)
        )
        return factor * float(np.sum(losses)), factor * gradient

    def prepare_lidar_tooth_pose_correspondences(self):
        """Freeze field-local nearest neighbours for one Stage-2 SLSQP pass."""
        ...
        for original_index, constraints in self.lidar_tooth_pose_constraints.items():
            for constraint in constraints:
                camera, _ = self._lidar_tooth_camera_points(
                    constraint, self.scales, self.rotVecXYZs, self.transVecXYZs
                )
                if camera is None:
                    continue
                configuration = constraint.configuration
                raw_pair = build_fixed_correspondences(camera, constraint, self.lidar_metric_scale)
                raw_count = int(len(raw_pair.model_indices))
                pair = raw_pair
                diagnostic = {
                    "mode": "raw",
                    "rawPairCount": raw_count,
                    "bootstrapPairCount": 0,
                    "pairCount": raw_count,
                }
                if raw_count < configuration.minimum_bootstrap_pair_count:
                    bootstrap = estimate_correspondence_bootstrap_translation(
                        camera,
                        constraint.points_k0_metres,
                        self.lidar_metric_scale,
                        configuration.maximum_bootstrap_translation_metres,
                    )
                    if bootstrap is None:
                        attempted = estimate_correspondence_bootstrap_translation(
                            camera,
                            constraint.points_k0_metres,
                            self.lidar_metric_scale,
                        ...
                        pair = FixedLiDARCorrespondences(empty, empty)
                        diagnostic = {
                            "mode": "disabled",
                            "rawPairCount": raw_count,
                            "bootstrapPairCount": 0,
                            "pairCount": 0,
                            "rejectionReason": "bootstrap_translation_rejected",
                        }
                        if attempted is not None:
                            diagnostic["translationMetres"] = attempted.translation_metres.tolist()
                            diagnostic["rejectionReason"] = "bootstrap_translation_exceeds_maximum"
                    else:
                        candidate = build_fixed_correspondences(
                            camera,
                            constraint,
                            self.lidar_metric_scale,
                            query_translation_metres=bootstrap.translation_metres,
                        )
                        candidate_count = int(len(candidate.model_indices))
                        diagnostic = {
                            "mode": "translation_bootstrap",
                            "translationMetres": bootstrap.translation_metres.tolist(),
                            "rawPairCount": raw_count,
                            "bootstrapPairCount": candidate_count,
                            "pairCount": candidate_count,
                        }
                        if candidate_count >= configuration.minimum_bootstrap_pair_count:
                            pair = candidate
                        else:
                            empty = np.empty(0, dtype=np.intp)
                            pair = FixedLiDARCorrespondences(empty, empty)
                            diagnostic["mode"] = "disabled"
                            diagnostic["pairCount"] = 0
                            diagnostic["rejectionReason"] = "insufficient_bootstrap_pairs"
                pairs[(original_index, constraint.field)] = pair
                key = f"{constraint.slot_id}@{constraint.field}"
                counts[key] = int(len(pair.model_indices))
                alignment_diagnostics[key] = diagnostic
        self.lidar_tooth_pose_correspondences = pairs
        self.lidar_tooth_pose_diagnostics["pairCounts"] = counts
        self.lidar_tooth_pose_diagnostics["correspondenceAlignment"] = alignment_diagnostics
        self.lidar_tooth_pose_diagnostics["noInGateCorrespondences"] = not any(counts.values())

    def compute_lidar_tooth_pose_loss_and_gradient(
        self, scales, rot_vecs, trans_vecs, pIdx, param_count, step
    ):
        """Fixed-pair Huber loss affecting only one Stage-2 pose parameter block."""

        gradient = np.zeros((param_count,), dtype=np.float64)
        if step not in (1, 2) or not self.lidar_tooth_pose_constraints:
            return 0.0, gradient
        losses = []
        axes = np.array([1.0, -1.0, -1.0], dtype=np.float64)
        for original_index, constraints in self.lidar_tooth_pose_constraints.items():
            for constraint in constraints:
                pair = self.lidar_tooth_pose_correspondences.get((original_index, constraint.field))
                if pair is None or len(pair.model_indices) == 0:
                    continue
                camera, state = self._lidar_tooth_camera_points(constraint, scales, rot_vecs, trans_vecs)
                if camera is None:
                    continue
                tooth_index, local_points, lower_in_shared_view = state
                aligned = arkit_points_from_em_camera(camera[pair.model_indices], self.lidar_metric_scale)
                loss, point_gradient = huber_point_to_surface_loss(
                    aligned,
                    constraint.points_k0_metres[pair.lidar_indices],
                    constraint.configuration.huber_delta_metres,
                ...
                local_gradient = (point_gradient * axes * self.lidar_metric_scale) @ extrinsic[:3, :].T
                if lower_in_shared_view:
                    local_gradient = local_gradient @ self.updateRelaRotMat(self.rela_rxyz).T
                if step == 1:
                    start = pIdx["tXYZs"] + 3 * tooth_index
                    gradient[start : start + 3] += local_gradient.sum(axis=0)
                else:
                    start = pIdx["rXYZs"] + 3 * tooth_index
                    relative = local_points[pair.model_indices] - self.X_Mu_centroids[tooth_index]
                    gradient[start : start + 3] += np.matmul(
                        local_gradient[:, None, :], -self.skewMatrices(relative)
                    ).sum(axis=0).ravel()
        if not losses:
            return 0.0, gradient
        configuration = next(iter(next(iter(self.lidar_tooth_pose_constraints.values())))).configuration
        ...
        self.lidar_tooth_pose_diagnostics["lastLossMetresSquared"] = float(np.mean(losses))
        return factor * float(np.sum(losses)), factor * gradient

    def _lidar_model_camera_points(self, photo_index, ex_rxyz, ex_txyz, row_scale_xz):
        """Return stage-0/1 SSM samples in one EM camera frame and pre-scale world points."""

        tooth_indices = self.visIdx[photo_index]
        per_tooth = [np.asarray(points, dtype=np.float64) for points in self.X_deformed[tooth_indices]]
        if photo_index in (
            PHOTO.UPPER.value,
            PHOTO.LOWER.value,
            PHOTO.LEFT.value,
            ...
            upper_count = self.ul_sp[photo_index]
            relative_rotation = self.updateRelaRotMat(self.rela_rxyz)
            per_tooth = per_tooth[:upper_count] + [
                self.updateLowerPointPosByRelaPose(points, relative_rotation, self.rela_txyz)
                for points in per_tooth[upper_count:]
            ]
        world_before_row_scale = np.vstack(per_tooth)
        row_scale = np.array([row_scale_xz[0], 1.0, row_scale_xz[1]], dtype=np.float64)
        world = world_before_row_scale * row_scale
        camera = self.updatePointPosInCameraCoord(
            world, self.updateExtrinsicViewMatrix(ex_rxyz[photo_index], ex_txyz[photo_index])
        )
        return camera, world_before_row_scale

    def prepare_lidar_correspondences(self, bootstrap_mode: str = "translation"):
        """Build fixed pairs with a robust query-only bootstrap when needed.

        `bootstrap_mode="translation"` (default) uses the median-to-median
        translation-only bootstrap. `bootstrap_mode="rigid"` uses a trimmed
        rotation+translation coarse-ICP estimate instead — offered as an
        alternate M5 candidate for `select_best_coarse_lidar_candidate` to
        choose between, not a replacement for the default.
        """

        if bootstrap_mode not in ("translation", "rigid"):
            raise ValueError(f"unknown bootstrap_mode: {bootstrap_mode!r}")
        if not self.lidar_constraints:
            return
        pairs = {}
        pair_counts = {}
        alignment_diagnostics = {}
        for photo_index, constraint in self.lidar_constraints.items():
            camera, _ = self._lidar_model_camera_points(
                photo_index, self.ex_rxyz, self.ex_txyz, self.rowScaleXZ
            )
            configuration = constraint.configuration
            raw_pair = build_fixed_correspondences(camera, constraint, self.lidar_metric_scale)
            raw_count = int(len(raw_pair.model_indices))
            ...
            pair = raw_pair
            if raw_count < configuration.minimum_bootstrap_pair_count:
                if bootstrap_mode == "rigid":
                    pair, diagnostic = self._lidar_rigid_bootstrap_correspondences(
                        camera, constraint, configuration, raw_count
                    )
                else:
                    pair, diagnostic = self._lidar_translation_bootstrap_correspondences(
                        camera, constraint, configuration, raw_count
                    )
            pairs[photo_index] = pair
            pair_counts[constraint.field] = int(len(pair.model_indices))
            alignment_diagnostics[constraint.field] = diagnostic
        self.lidar_correspondences = pairs
        self.lidar_diagnostics["pairCounts"] = pair_counts
        self.lidar_diagnostics["correspondenceAlignment"] = alignment_diagnostics
        self.lidar_diagnostics["noInGateCorrespondences"] = not any(pair_counts.values())
        if "photoOnlyMedianDistanceMillimetres" not in self.lidar_diagnostics:
            self._record_lidar_distance_diagnostics(prefix="photoOnly")

    def _lidar_translation_bootstrap_correspondences(self, camera, constraint, configuration, raw_count):
        bootstrap = estimate_correspondence_bootstrap_translation(
            camera,
            constraint.points_k0_metres,
            self.lidar_metric_scale,
        ...
        if bootstrap is None:
            ...
            if attempted is not None:
                diagnostic["translationMetres"] = attempted.translation_metres.tolist()
                diagnostic["rejectionReason"] = "bootstrap_translation_exceeds_maximum"
            return FixedLiDARCorrespondences(empty, empty), diagnostic
        candidate = build_fixed_correspondences(
            camera,
            constraint,
            self.lidar_metric_scale,
    ...
    def _lidar_rigid_bootstrap_correspondences(self, camera, constraint, configuration, raw_count):
        bootstrap = estimate_correspondence_bootstrap_rigid_transform(
            camera,
            constraint.points_k0_metres,
            self.lidar_metric_scale,
            configuration,
        ...
        if bootstrap is None:
            unbounded_configuration = dataclasses.replace(
                configuration,
                maximum_bootstrap_translation_metres=np.finfo(np.float64).max,
                maximum_bootstrap_rotation_radians=np.finfo(np.float64).max,
            )
            attempted = estimate_correspondence_bootstrap_rigid_transform(
                camera,
                constraint.points_k0_metres,
                self.lidar_metric_scale,
                unbounded_configuration,
            )
            empty = np.empty(0, dtype=np.intp)
            diagnostic = {
                "mode": "disabled",
                ...
                "bootstrapPairCount": 0,
                "pairCount": 0,
                "rejectionReason": "bootstrap_rigid_transform_rejected",
            }
            if attempted is not None:
                diagnostic["translationMetres"] = attempted.translation_metres.tolist()
                diagnostic["rejectionReason"] = "bootstrap_rigid_transform_exceeds_maximum"
            return FixedLiDARCorrespondences(empty, empty), diagnostic
        candidate = build_fixed_correspondences(
            camera,
            constraint,
            self.lidar_metric_scale,
            query_translation_metres=bootstrap.translation_metres,
            query_rotation=bootstrap.rotation,
        )
        candidate_count = int(len(candidate.model_indices))
        diagnostic = {
            "mode": "rigid_bootstrap",
            "translationMetres": bootstrap.translation_metres.tolist(),
            "rotation": bootstrap.rotation.tolist(),
            "rawPairCount": raw_count,
            "bootstrapPairCount": candidate_count,
            "pairCount": candidate_count,
        }
    ...
    def _lidar_pair_distances_metres(self):
        distances = []
        for photo_index, constraint in self.lidar_constraints.items():
            pair = self.lidar_correspondences.get(photo_index)
            if pair is None or len(pair.model_indices) == 0:
                continue
            camera, _ = self._lidar_model_camera_points(
                photo_index, self.ex_rxyz, self.ex_txyz, self.rowScaleXZ
            ...
            aligned = arkit_points_from_em_camera(
                camera[pair.model_indices], self.lidar_metric_scale
            )
            target = constraint.points_k0_metres[pair.lidar_indices]
            distances.append(np.linalg.norm(aligned - target, axis=1))
        return np.concatenate(distances) if distances else np.empty(0, dtype=np.float64)

    def _record_lidar_distance_diagnostics(self, prefix):
        distances = self._lidar_pair_distances_metres()
        if len(distances) == 0:
            return
        self.lidar_diagnostics[f"{prefix}MedianDistanceMillimetres"] = float(
            np.median(distances) * 1000.0
        )
        self.lidar_diagnostics[f"{prefix}P95DistanceMillimetres"] = float(
            np.percentile(distances, 95) * 1000.0
        )

    def finalize_lidar_diagnostics(self):
        """Measure the selected final global solution without reopening later stages."""

        if not self.lidar_constraints:
            return
        self._record_lidar_distance_diagnostics(prefix="coarseLiDAR")
        self.lidar_diagnostics["stage2PoseParameterDeltaFromCoarseLiDAR"] = 0.0
        self.lidar_diagnostics["stage3FeatureVectorDeltaFromCoarseLiDAR"] = 0.0

    def compute_lidar_coarse_loss_and_gradient(
        self, ex_rxyz, ex_txyz, row_scale_xz, metres_per_ssm_unit, pIdx, param_count, stage
    ):
        """Fixed-pair Huber loss with derivatives limited to stage-0/1 globals."""

        gradient = np.zeros((param_count,), dtype=np.float64)
        if stage not in (0, 1) or not self.lidar_constraints:
            return 0.0, gradient
        losses = []
        contributing = 0
        axes = np.array([1.0, -1.0, -1.0], dtype=np.float64)
        for photo_index, constraint in self.lidar_constraints.items():
            ...
            if pair is None or len(pair.model_indices) == 0:
                continue
            camera, world_before_row_scale = self._lidar_model_camera_points(
                photo_index, ex_rxyz, ex_txyz, row_scale_xz
            )
            selected_camera = camera[pair.model_indices]
            aligned = arkit_points_from_em_camera(selected_camera, metres_per_ssm_unit)
            loss, point_gradient = huber_point_to_surface_loss(
                aligned,
                constraint.points_k0_metres[pair.lidar_indices],
                constraint.configuration.huber_delta_metres,
            ...
            losses.append(loss)
            contributing += 1
            camera_gradient = point_gradient * axes * metres_per_ssm_unit
            rotation_gradient = np.matmul(
                camera_gradient[:, None, :], -self.skewMatrices(selected_camera)
            ).sum(axis=0).ravel()
            rotation_start = pIdx["ex_rxyz"] + 3 * photo_index
            translation_start = pIdx["ex_txyz"] + 3 * photo_index
            gradient[rotation_start : rotation_start + 3] += rotation_gradient
            gradient[translation_start : translation_start + 3] += camera_gradient.sum(axis=0)
            if stage == 1:
                rotation = RR.from_rotvec(ex_rxyz[photo_index]).as_matrix().T
                source = world_before_row_scale[pair.model_indices]
                for axis_index, source_index in enumerate((0, 2)):
                    derivative_camera = np.zeros_like(source)
                    derivative_camera[:, source_index] = source[:, source_index]
                    derivative_arkit = np.matmul(derivative_camera, rotation) * axes * metres_per_ssm_unit
                    gradient[pIdx["rowScaleXZ"] + axis_index] += np.sum(point_gradient * derivative_arkit)
            gradient[pIdx["lidarMetricScale"]] += np.sum(point_gradient * selected_camera * axes)
        if not losses:
            self.lidar_diagnostics["noInGateCorrespondences"] = True
            return 0.0, gradient
        weight = next(iter(self.lidar_constraints.values())).configuration.weight
        factor = (
            weight
            * METRES_SQUARED_TO_MILLIMETRES_SQUARED
            / contributing
        )
        self.lidar_diagnostics["lastLossMetresSquared"] = float(np.mean(losses))
        self.lidar_diagnostics["metricScaleMetresPerSSMUnit"] = float(metres_per_ssm_unit)
        return factor * float(np.sum(losses)), factor * gradient
    ...
    def updateCameraParams(
        ...
        ph = phType.value
        if phType in [PHOTO.UPPER, PHOTO.LOWER, PHOTO.LEFT, PHOTO.RIGHT, PHOTO.FRONTAL]:
            ul_sp = self.ul_sp[ph]
    ...
    def gridSearchExtrinsicParams(self):
        ...
        for phType, paramDict in ExtrParamSearchSpace.items():
            ...
            for paramName, paramSearchSpace in paramDict.items():
                ...
                TY_list = [
                        self.rigid_registration_2D(self.P_true[ph], _P_pred)
    ...
    def relativePoseParamSearchSpace(self):
        # search appropriate values for rela_rxyz, rela_txyz.
        # rela.r.* has no local counterpart search elsewhere: SLSQP only
        # refines rela_rxyz +/-0.05 rad around its starting value, so a real
        # crossbite rotation outside that window is otherwise never found.
        # Occlusal views (looking straight down/up at the arch) show a
        # left/right crossbite most directly, so they score the rotation
        # candidates here.
        rotation_offsets = np.array([-0.3, -0.2, -0.1, 0.0, 0.1, 0.2, 0.3])
        return {
            "rela.t.z": (
            ...
            "rela.r.x": (
                self.rela_rxyz_default[0] + rotation_offsets,
                [PHOTO.UPPER, PHOTO.LOWER],
            ),
            "rela.r.y": (
                self.rela_rxyz_default[1] + rotation_offsets,
                [PHOTO.UPPER, PHOTO.LOWER],
            ),
            "rela.r.z": (
                self.rela_rxyz_default[2] + rotation_offsets,
                [PHOTO.UPPER, PHOTO.LOWER],
            ),
        }

    def gridSearchRelativePoseParams(self):
        RelativePoseParamSearchSpace = self.relativePoseParamSearchSpace()
        for paramName, (
            paramSearchSpace,
            phTypes,
            ...
            TY_list = [
                    self.rigid_registration_2D(_P_true, _P_pred)
    ...
    def updateAlignedPointCloudInWorldCoord(self, stage, tIdx):
        # Stage 0/1 geometry must be rebuilt from the immutable mean shape.
        # Reusing X_trans here compounds row scaling and lets a rejected M5
        # candidate contaminate a later photo-only rollback.
        if stage < 2:
            self.X_trans[tIdx] = self.X_Mu[tIdx]
            self.X_trans_normals[tIdx] = self.X_Mu_normals[tIdx]
        if stage >= 3:
    ...
    def updateEdgePrediction(self, photoType):
        ...
        if photoType in [PHOTO.UPPER, PHOTO.LOWER, PHOTO.LEFT, PHOTO.RIGHT, PHOTO.FRONTAL]:
            ul_sp = self.ul_sp[ph]
            self.rela_R = self.updateRelaRotMat(self.rela_rxyz)
    ...
    def renderPredictedEdgeMask(self, photoType, canvasShape=None, dilate=True):
        """Standalone white-line-on-black mask of just the reprojected/predicted
        teeth edges (self.P_pred), in the same pixel space as the ground-truth
        edge mask (self.edgeMask) fed to the optimizer. For LEFT/RIGHT/FRONTAL
        this already reflects the fitted RELA_R/RELA_T bite-relative pose
        (applied in updateEdgePrediction), so it can be overlaid on that photo
        to visually check bite alignment. Ported from emopt5views.py so the
        LiDAR engine's post-reconstruction QA overlay (main.py) doesn't crash.
        """
        ph = photoType.value
        ...
        # A still-converging pose (e.g. occlusal views, only recently wired
        # into bite fitting) can predict points outside the ground-truth
        # edge mask's pixel bounds; clip rather than crash on this
        # visualization/QA path.
        u = np.clip(pix_pred[:, 0], 0, canvasShape[1] - 1)
        v = np.clip(pix_pred[:, 1], 0, canvasShape[0] - 1)
        edgePred[v, u] = 1.0
        if dilate:
            edgePred = skimage.morphology.binary_dilation(
                edgePred, skimage.morphology.disk(2)
            )
        return (edgePred * 255).astype(np.uint8)
    ...
    def getCurrentGlobalParamsOf5Views_as_x0(self, stage, step):
        ...
        if stage in (0, 1) and self.lidar_constraints:
            pIdx["lidarMetricScale"] = len(x0)
            x0 = np.hstack([x0, self.lidar_metric_scale])
        return x0, pIdx
    ...
    def MStepLoss(self, params, pIdx, stage, step, verbose, return_grad=False):
        ...
        if stage in (0, 1) and self.lidar_constraints:
            metres_per_ssm_unit = params[pIdx["lidarMetricScale"]]
            lidar_loss, lidar_gradient = self.compute_lidar_coarse_loss_and_gradient(
                ex_rxyz,
                ex_txyz,
                rowScaleXZ,
                metres_per_ssm_unit,
            ...
            M_loss += lidar_loss
            if return_grad:
                M_grad += lidar_gradient
        if stage == 2 and step in (1, 2) and self.lidar_tooth_pose_constraints:
            tooth_pose_loss, tooth_pose_gradient = self.compute_lidar_tooth_pose_loss_and_gradient(
                scales, rotVecXYZs, transVecXYZs, pIdx, paramNum, step
            )
            M_loss += tooth_pose_loss
            if return_grad:
                M_grad += tooth_pose_gradient
        if stage == 3 and self.lidar_tooth_shape_constraints:
            shape_loss, shape_gradient = self.compute_lidar_tooth_shape_loss_and_gradient(
                featureVec, pIdx, paramNum, stage
            )
            M_loss += shape_loss
            if return_grad:
                M_grad += shape_gradient
        if verbose == True:
            print(
    ...
    def maximization_step_5Views(self, stage, step, maxiter=100, verbose=True):
        ...
        if stage in (0, 1) and self.lidar_constraints:
            self.prepare_lidar_correspondences()
        if stage == 2 and step in (1, 2) and self.lidar_tooth_pose_constraints:
            self.prepare_lidar_tooth_pose_correspondences()
        if stage == 3:
            self.prepare_lidar_tooth_shape_correspondences()
            for phType in self.photoTypes:
        ...
        if stage in (0, 1) and self.lidar_constraints:
            self.lidar_metric_scale = float(params[pIdx["lidarMetricScale"]])
        elif stage == 2:
            ...
            self.update_lidar_tooth_shape_diagnostics()
        self.loss_maximization_step = self.MStepLoss(
            params, pIdx, stage, step, verbose, return_grad=False
        )

    def maximization_lidar_tooth_shape_only(self, maxiter=100, verbose=False):
        """Run the gated M7 pass without reopening camera or neighboring parameters."""

        if not self.lidar_tooth_shape_constraints:
            return
        base_params, pIdx = self.getCurrentGlobalParamsOf5Views_as_x0(3, -1)
        feature_start = pIdx["featureVec"]
        original_indices = np.flatnonzero(self.Mask)
        selected = []
        for original_index in self.lidar_tooth_shape_constraints:
            matches = np.flatnonzero(original_indices == original_index)
            if len(matches) != 1:
                continue
            tooth_index = int(matches[0])
            selected.extend(
                range(
                    feature_start + tooth_index * self.numPC,
                    feature_start + (tooth_index + 1) * self.numPC,
                )
            )
        selected = np.asarray(selected, dtype=np.intp)
        if len(selected) == 0:
            return
        all_bounds = self.getParamBounds(base_params, pIdx, 3, -1)
        selected_bounds = [all_bounds[index] for index in selected]

        def selected_loss(values):
            params = np.asarray(base_params, dtype=np.float64).copy()
            params[selected] = values
            loss, gradient = self.MStepLoss(
                params, pIdx, 3, -1, False, return_grad=True
            )
            return loss, gradient[selected]

        result = scipy.optimize.minimize(
            fun=selected_loss,
            x0=base_params[selected],
            jac=True,
        ...
        final_params = np.asarray(base_params, dtype=np.float64).copy()
        final_params[selected] = result.x
        self.featureVec = final_params[
            feature_start : feature_start + self.numTooth * self.numPC
        ].reshape(self.featureVec.shape)
        self.update_lidar_tooth_shape_diagnostics()
        self.loss_maximization_step = self.MStepLoss(
            final_params, pIdx, 3, -1, verbose, return_grad=False
        )
    ...
    def getParamBounds(self, x0, pIdx, stage, step):
        ...
        if stage in (0, 1) and self.lidar_constraints:
            configuration = next(iter(self.lidar_constraints.values())).configuration
            bounds += [configuration.metres_per_ssm_unit_bounds]
        return bounds
    ...
    def saveDemo2H5(self, h5File):
        ...
        with h5py.File(h5File, "w") as f:  # ovewrite file each time
            ...
            grp.create_dataset("NUM_PC", data=self.numPC)
            grp.create_dataset(
                "SHAPE_REGULARIZATION", data=self.weightFeatureVec
            )
            grp.attrs["LIDAR_CONSTRAINTS_JSON"] = json.dumps(self.lidar_diagnostics, sort_keys=True)
            grp.attrs["LIDAR_TOOTH_POSE_CONSTRAINTS_JSON"] = json.dumps(
                self.lidar_tooth_pose_diagnostics, sort_keys=True
            )
            grp.attrs["LIDAR_TOOTH_SHAPE_CONSTRAINTS_JSON"] = json.dumps(
                self.lidar_tooth_shape_diagnostics, sort_keys=True
            )
            write_lidar_h5_diagnostics(
                grp,
                {
                    "coarse": self.lidar_diagnostics,
                    "toothPose": self.lidar_tooth_pose_diagnostics,
                    "toothShape": self.lidar_tooth_shape_diagnostics,
                },
            )
