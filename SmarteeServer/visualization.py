...
def meshProjection(visualizer, tag):
    ...
    for phType, img in zip(PHOTO_TYPES, photos):
        ...
        skimage.io.imsave(mesh_img_file, skimage.img_as_ubyte(np.clip(mshImg, 0, 1)))
        ...
        skimage.io.imsave(output_img_file, skimage.img_as_ubyte(np.clip(output, 0, 1)))
...
if __name__ == "__main__":
    for tag in ["3", "4", "5"]:
        print(f"Generating visualization for patient {tag}...")
        main(tag=tag)
        print(f"✅ Patient {tag} done.")
