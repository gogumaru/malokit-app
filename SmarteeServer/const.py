...
# ponytail: defaultdict instead of a plain dict — any tag not explicitly listed
# here (e.g. one generated per server request) falls back to "all 28 teeth
# present" instead of raising KeyError, so patients no longer need manual
# registration to run reconstruction.
TOOTH_EXIST_MASK = collections.defaultdict(
    lambda: np.ones((28,), np.bool_),
    {
        "6": np.ones((28,), np.bool_)
    },
)
...
RECONSTRUCTION_DATA_DIR = r"./demo/reconstruction_data/"
