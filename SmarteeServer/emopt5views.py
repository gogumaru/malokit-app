...
class EMOpt5Views(object):
    ...
    def renderPredictedEdgeMask(self, photoType, canvasShape=None, dilate=True):
        ...
        edge mask (self.edgeMask) fed to the optimizer. Non-interactive sibling
        of showEdgeMaskPredictionWithGroundTruth, for saving to disk instead of
        plt.imshow. For LEFT/RIGHT/FRONTAL this already reflects the fitted
        RELA_R/RELA_T bite-relative pose (applied in updateEdgePrediction), so
        it can be overlaid on that photo to visually check bite alignment.
        """
        ph = photoType.value
        if not bool(canvasShape):
            canvasShape = self.edgeMask[ph].shape
